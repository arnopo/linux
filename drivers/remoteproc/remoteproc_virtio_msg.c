...
static void rproc_vmsg_mb_callback(struct mbox_client *cl, void *data)
{
	...
	while (!queue_receive(rvmdev_bus, &msg, sizeof(msg))) {
		...
		if (!rvmdev) {
			dev_err(&rvmdev_bus->pdev->dev, "unknown virtio message device");
			return;
		}
	...
	of_for_each_phandle(&it, ret, dev->of_node, "memory-region", NULL, 0)	{
		...
		if (!index) {
			...
			if (IS_ERR_OR_NULL(rvmdev_bus->shmem_queue)) {
				dev_err(dev, "Unable to map memory region: %pa[p]+%pa\n",
					&rmem->base, &rmem->size);
				return -ENOMEM;
			}
		} else if (index == 1) {
			...
			if (ret) {
				dev_err(dev, "Unable to declare coherent memory: %pa+%pa\n",
					&rmem->base, &rmem->size);
				return ret;
			}
		} else {
...
static int rproc_vmsg_of_parse_vdev(struct rproc_vmdev_bus *rvmdev_bus)
{
	...
	for_each_child_of_node(dev->of_node, child_np) {
		...
		rvmdev->vmdev.vdev.dev.of_node = child_np;
...
static int rproc_vmsg_probe(struct platform_device *pdev)
{
	...
	rvmdev_bus = devm_kzalloc(dev, sizeof(*rvmdev_bus), GFP_KERNEL);
	if (!rvmdev_bus)
		return -ENOMEM;
